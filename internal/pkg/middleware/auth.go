...
func AuthRequired() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.AbortWithStatusJSON(401, errors.New(errors.ErrUnauthorized, err.Error()))
			return
		}
		...
		if err := database.DB.First(&device, claims.DeviceID).Error; err != nil {
			c.AbortWithStatusJSON(401, errors.New(errors.ErrUnauthorized, "device not found"))
			return
		}

		// 检查设备状态
		if device.Status != 1 { // 假设1表示正常状态
			c.AbortWithStatusJSON(401, errors.New(errors.ErrUnauthorized, "device is disabled"))
			return
		}
