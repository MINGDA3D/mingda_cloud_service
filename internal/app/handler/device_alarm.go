...
func (h *DeviceAlarmHandler) ReportDeviceAlarm(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Error(c, errors.New(errors.ErrInvalidParams, "参数绑定错误"))
		return
	}
...
func (h *DeviceAlarmHandler) ResolveAlarm(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, errors.New(errors.ErrInvalidParams, "无效的告警ID"))
		return
	}
...
func (h *DeviceAlarmHandler) GetDeviceAlarms(c *gin.Context) {
	...
	if statusStr := c.Query("status"); statusStr != "" {
		...
		if err != nil {
			response.Error(c, errors.New(errors.ErrInvalidParams, "无效的状态参数"))
			return
		}
		status = &statusVal
	}
